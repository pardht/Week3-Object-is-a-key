...
function countProfit(shoppers) {
    ...
    for (let i = 0; i < listBarang.length; i++) {
        let leftOvers = listBarang[i][2];
        let totalProfit = 0;
        let shopper = [];

        for (let i2 = 0; i2 < shoppers.length; i2++) {
            if(listBarang[i][0] === shoppers[i2].product && listBarang[i][2] >= shoppers[i2].amount) {
                leftOvers = leftOvers - shoppers[i2].amount;
                listBarang[i][2] = leftOvers;
                totalProfit += shoppers[i2].amount * listBarang[i][1];
                shopper.push(shoppers[i2].name)
            }
        }
